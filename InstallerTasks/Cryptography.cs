...
namespace InstallationTasks
{
	...
	****************************************************************************/
        ...
        public const int ALGO_LAST = 305; // Last possible Cryptography algo value
        ...
        public const int MODE_LAST = 5; // Last possible Cryptography mode value
	...
	****************************************************************************/
        ...
        public const int CERTINFO_QCSTATEMENT_REGISTRATIONAUTHORITY = 2216;
            // qcStatement.statementInfo.nameRegistrationAuthorities

        public const int CERTINFO_IPADDRESSBLOCKS = 2217;

        public const int CERTINFO_IPADDRESSBLOCKS_ADDRESSFAMILY = 2218;
            // addressFamily */	CRYPT_CERTINFO_IPADDRESSBLOCKS_PREFIX,	/* ipAddress.addressPrefix
        ...
        public const int CERTINFO_CERTPOLICY_ORGANIZATION = 2297;
            // policyInformation.policyQualifiers.qualifier.userNotice.noticeRef.organization

        public const int CERTINFO_CERTPOLICY_NOTICENUMBERS = 2298;
            // policyInformation.policyQualifiers.qualifier.userNotice.noticeRef.noticeNumbers

        public const int CERTINFO_CERTPOLICY_EXPLICITTEXT = 2299;
        ...
        public const int CERTINFO_CMS_MLEXP_INSTEADOF = 2541;
            // mlData.mlReceiptPolicy.insteadOf.generalNames.generalName

        public const int CERTINFO_CMS_MLEXP_INADDITIONTO = 2542;
        ...
        public const int CERTINFO_CMS_SIGPOLICY_ORGANIZATION = 2562;
            // sigPolicyQualifiers.sigPolicyQualifier.userNotice.noticeRef.organization

        public const int CERTINFO_CMS_SIGPOLICY_NOTICENUMBERS = 2563;
            // sigPolicyQualifiers.sigPolicyQualifier.userNotice.noticeRef.noticeNumbers

        public const int CERTINFO_CMS_SIGPOLICY_EXPLICITTEXT = 2564;
	...
	****************************************************************************/
        ...
        {
            var cryptQueryInfoPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof (CRYPT_QUERY_INFO)));
            var cryptQueryInfo = new CRYPT_QUERY_INFO();
            try
            {
        ...
        {
            var cryptContextPtr = Marshal.AllocHGlobal(4);
            try
            {
        ...
        {
            SetAttributeString(cryptHandle, attributeType, value == null ? null : new UTF8Encoding().GetBytes(value), 0,
                value == null ? 0 : new UTF8Encoding().GetByteCount(value));
        }
        ...
        {
            var length = GetAttributeString(cryptHandle, attributeType, null);
	   ...
	   be found */
        ...
        {
            var randomDataHandle = new GCHandle();
            var randomDataPtr = IntPtr.Zero;
            try
            {
        ...
        {
            AddRandom(randomData == null ? null : new UTF8Encoding().GetBytes(randomData), 0,
                randomData == null ? 0 : new UTF8Encoding().GetByteCount(randomData));
        }
        ...
        {
            var cryptObjectInfoPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof (CRYPT_OBJECT_INFO)));
            var cryptObjectInfo = new CRYPT_OBJECT_INFO();
            var objectDataHandle = new GCHandle();
            var objectDataPtr = IntPtr.Zero;
            try
            {
        ...
        {
            return QueryObject(objectData == null ? null : new UTF8Encoding().GetBytes(objectData), 0,
                objectData == null ? 0 : new UTF8Encoding().GetByteCount(objectData));
        }
	...
	****************************************************************************/
        ...
        {
            var encryptedKeyLengthPtr = Marshal.AllocHGlobal(4);
            var encryptedKeyHandle = new GCHandle();
            var encryptedKeyPtr = IntPtr.Zero;
            try
            {
                processStatus(wrapped_ExportKey(encryptedKeyPtr, encryptedKeyMaxLength, encryptedKeyLengthPtr, exportKey,
                    sessionKeyContext));
                var encryptedKeyLength = Marshal.ReadInt32(encryptedKeyLengthPtr);
            ...
            )
        {
            ...
            try
            {
                processStatus(wrapped_ExportKeyEx(encryptedKeyPtr, encryptedKeyMaxLength, encryptedKeyLengthPtr,
        ...
        {
            ImportKey(encryptedKey == null ? null : new UTF8Encoding().GetBytes(encryptedKey), 0,
                encryptedKey == null ? 0 : new UTF8Encoding().GetByteCount(encryptedKey), importKey, sessionKeyContext);
        }
        ...
        {
            var returnedContextPtr = Marshal.AllocHGlobal(4);
            var encryptedKeyHandle = new GCHandle();
            var encryptedKeyPtr = IntPtr.Zero;
            try
            {
                checkIndices(encryptedKey, encryptedKeyOffset, encryptedKeyLength);
                getPointer(encryptedKey, encryptedKeyOffset, ref encryptedKeyHandle, ref encryptedKeyPtr);
                processStatus(wrapped_ImportKeyEx(encryptedKeyPtr, encryptedKeyLength, importKey, sessionKeyContext,
                    returnedContextPtr));
        ...
        {
            return ImportKeyEx(encryptedKey, 0, encryptedKey == null ? 0 : encryptedKey.Length, importKey,
                sessionKeyContext);
        }
        ...
        {
            return ImportKeyEx(encryptedKey == null ? null : new UTF8Encoding().GetBytes(encryptedKey), 0,
                encryptedKey == null ? 0 : new UTF8Encoding().GetByteCount(encryptedKey), importKey, sessionKeyContext);
        }
        ...
        {
            var signatureLengthPtr = Marshal.AllocHGlobal(4);
            var signatureHandle = new GCHandle();
            var signaturePtr = IntPtr.Zero;
            try
            {
                processStatus(wrapped_CreateSignature(signaturePtr, signatureMaxLength, signatureLengthPtr, signContext,
                    hashContext));
                var signatureLength = Marshal.ReadInt32(signatureLengthPtr);
            ...
            )
        {
            ...
            try
            {
                processStatus(wrapped_CreateSignatureEx(signaturePtr, signatureMaxLength, signatureLengthPtr, formatType,
        ...
        {
            CheckSignature(signature == null ? null : new UTF8Encoding().GetBytes(signature), 0,
                signature == null ? 0 : new UTF8Encoding().GetByteCount(signature), sigCheckKey, hashContext);
        }
        ...
        {
            var extraDataPtr = Marshal.AllocHGlobal(4);
            var signatureHandle = new GCHandle();
            var signaturePtr = IntPtr.Zero;
            try
            {
                checkIndices(signature, signatureOffset, signatureLength);
                getPointer(signature, signatureOffset, ref signatureHandle, ref signaturePtr);
                processStatus(wrapped_CheckSignatureEx(signaturePtr, signatureLength, sigCheckKey, hashContext,
                    extraDataPtr));
        ...
        {
            return CheckSignatureEx(signature == null ? null : new UTF8Encoding().GetBytes(signature), 0,
                signature == null ? 0 : new UTF8Encoding().GetByteCount(signature), sigCheckKey, hashContext);
        }
	...
	****************************************************************************/
        ...
        {
            var keysetPtr = Marshal.AllocHGlobal(4);
            var nameHandle = new GCHandle();
            var namePtr = IntPtr.Zero;
            var nameArray = new UTF8Encoding().GetBytes(name);
            try
            {
        ...
        {
            var cryptContextPtr = Marshal.AllocHGlobal(4);
            var keyIDHandle = new GCHandle();
            var keyIDPtr = IntPtr.Zero;
            var keyIDArray = new UTF8Encoding().GetBytes(keyID);
            try
            {
            ...
            )
        {
            ...
            var keyIDPtr = IntPtr.Zero;
            var keyIDArray = new UTF8Encoding().GetBytes(keyID);
            var passwordHandle = new GCHandle();
            var passwordPtr = IntPtr.Zero;
            var passwordArray = new UTF8Encoding().GetBytes(password);
            try
            {
                getPointer(keyIDArray, 0, ref keyIDHandle, ref keyIDPtr);
	...
	****************************************************************************/
        ...
        {
            var certificatePtr = Marshal.AllocHGlobal(4);
            try
            {
	   ...
	   rather than C_STR */
        ...
        {
            var extensionLengthPtr = Marshal.AllocHGlobal(4);
            var criticalFlagPtr = Marshal.AllocHGlobal(4);
            var oidHandle = new GCHandle();
            var oidPtr = IntPtr.Zero;
            var oidArray = new UTF8Encoding().GetBytes(oid);
            var extensionHandle = new GCHandle();
            var extensionPtr = IntPtr.Zero;
            try
            {
                getPointer(oidArray, 0, ref oidHandle, ref oidPtr);
                processStatus(wrapped_GetCertExtension(certificate, oidPtr, criticalFlagPtr, extensionPtr,
                    extensionMaxLength, extensionLengthPtr));
                var extensionLength = Marshal.ReadInt32(extensionLengthPtr);
        ...
        {
            AddCertExtension(certificate, oid, criticalFlag,
                extension == null ? null : new UTF8Encoding().GetBytes(extension), 0,
                extension == null ? 0 : new UTF8Encoding().GetByteCount(extension));
        }
        ...
        {
            var certificatePtr = Marshal.AllocHGlobal(4);
            var certObjectHandle = new GCHandle();
            var certObjectPtr = IntPtr.Zero;
            try
            {
        ...
        {
            return ImportCert(certObject == null ? null : new UTF8Encoding().GetBytes(certObject), 0,
                certObject == null ? 0 : new UTF8Encoding().GetByteCount(certObject), cryptUser);
        }
        ...
        {
            var certObjectLengthPtr = Marshal.AllocHGlobal(4);
            var certObjectHandle = new GCHandle();
            var certObjectPtr = IntPtr.Zero;
            try
            {
                processStatus(wrapped_ExportCert(certObjectPtr, certObjectMaxLength, certObjectLengthPtr, certFormatType,
                    certificate));
                var certObjectLength = Marshal.ReadInt32(certObjectLengthPtr);
                checkIndices(certObject, certObjectOffset, certObjectLength);
                getPointer(certObject, certObjectOffset, ref certObjectHandle, ref certObjectPtr);
                processStatus(wrapped_ExportCert(certObjectPtr, certObjectMaxLength, certObjectLengthPtr, certFormatType,
                    certificate));
	...
	****************************************************************************/
        ...
        {
            var envelopePtr = Marshal.AllocHGlobal(4);
            try
            {
        ...
        {
            var sessionPtr = Marshal.AllocHGlobal(4);
            try
            {
        ...
        {
            var bytesCopiedPtr = Marshal.AllocHGlobal(4);
            var bufferHandle = new GCHandle();
            var bufferPtr = IntPtr.Zero;
            try
            {
        ...
        {
            return PushData(envelope, buffer == null ? null : new UTF8Encoding().GetBytes(buffer), 0,
                buffer == null ? 0 : new UTF8Encoding().GetByteCount(buffer));
        }
	...
	****************************************************************************/
        ...
        {
            var devicePtr = Marshal.AllocHGlobal(4);
            var nameHandle = new GCHandle();
            var namePtr = IntPtr.Zero;
            var nameArray = new UTF8Encoding().GetBytes(name);
            try
	...
	****************************************************************************/
        ...
        {
            var userPtr = Marshal.AllocHGlobal(4);
            var nameHandle = new GCHandle();
            var namePtr = IntPtr.Zero;
            var nameArray = new UTF8Encoding().GetBytes(name);
            var passwordHandle = new GCHandle();
	...
	****************************************************************************/
        ...
        private static extern int wrapped_SetAttributeString(int cryptHandle, int attributeType, IntPtr value,
            int valueLength);
        ...
        private static extern int wrapped_GetAttributeString(int cryptHandle, int attributeType, IntPtr value,
            IntPtr valueLength);
        ...
        private static extern int wrapped_ExportKey(IntPtr encryptedKey, int encryptedKeyMaxLength,
            IntPtr encryptedKeyLength, int exportKey, int sessionKeyContext);

        [DllImport("cl32.dll", EntryPoint = "cryptExportKeyEx")]
        private static extern int wrapped_ExportKeyEx(IntPtr encryptedKey, int encryptedKeyMaxLength,
            IntPtr encryptedKeyLength, int formatType, int exportKey, int sessionKeyContext);

        [DllImport("cl32.dll", EntryPoint = "cryptImportKey")]
        private static extern int wrapped_ImportKey(IntPtr encryptedKey, int encryptedKeyLength, int importKey,
            int sessionKeyContext);

        [DllImport("cl32.dll", EntryPoint = "cryptImportKeyEx")]
        private static extern int wrapped_ImportKeyEx(IntPtr encryptedKey, int encryptedKeyLength, int importKey,
            int sessionKeyContext, IntPtr returnedContext);

        [DllImport("cl32.dll", EntryPoint = "cryptCreateSignature")]
        private static extern int wrapped_CreateSignature(IntPtr signature, int signatureMaxLength,
            IntPtr signatureLength, int signContext, int hashContext);

        [DllImport("cl32.dll", EntryPoint = "cryptCreateSignatureEx")]
        private static extern int wrapped_CreateSignatureEx(IntPtr signature, int signatureMaxLength,
            IntPtr signatureLength, int formatType, int signContext, int hashContext, int extraData);

        [DllImport("cl32.dll", EntryPoint = "cryptCheckSignature")]
        private static extern int wrapped_CheckSignature(IntPtr signature, int signatureLength, int sigCheckKey,
            int hashContext);

        [DllImport("cl32.dll", EntryPoint = "cryptCheckSignatureEx")]
        private static extern int wrapped_CheckSignatureEx(IntPtr signature, int signatureLength, int sigCheckKey,
            int hashContext, IntPtr extraData);

        [DllImport("cl32.dll", EntryPoint = "cryptKeysetOpen")]
        private static extern int wrapped_KeysetOpen(IntPtr keyset, int cryptUser, int keysetType, IntPtr name,
            int options);
        ...
        private static extern int wrapped_GetPrivateKey(int keyset, IntPtr cryptContext, int keyIDtype, IntPtr keyID,
            IntPtr password);

        [DllImport("cl32.dll", EntryPoint = "cryptGetKey")]
        private static extern int wrapped_GetKey(int keyset, IntPtr cryptContext, int keyIDtype, IntPtr keyID,
            IntPtr password);
        ...
        private static extern int wrapped_GetCertExtension(int certificate, IntPtr oid, IntPtr criticalFlag,
            IntPtr extension, int extensionMaxLength, IntPtr extensionLength);

        [DllImport("cl32.dll", EntryPoint = "cryptAddCertExtension")]
        private static extern int wrapped_AddCertExtension(int certificate, IntPtr oid, int criticalFlag,
            IntPtr extension, int extensionLength);
        ...
        private static extern int wrapped_ImportCert(IntPtr certObject, int certObjectLength, int cryptUser,
            IntPtr certificate);

        [DllImport("cl32.dll", EntryPoint = "cryptExportCert")]
        private static extern int wrapped_ExportCert(IntPtr certObject, int certObjectMaxLength, IntPtr certObjectLength,
            int certFormatType, int certificate);
        ...
        private static extern int wrapped_CAGetItem(int keyset, IntPtr certificate, int certType, int keyIDtype,
            IntPtr keyID);
        ...
        private static extern int wrapped_CACertManagement(IntPtr certificate, int action, int keyset, int caKey,
            int certRequest);
    ...
    public class CRYPT_QUERY_INFO
    {
        ...
        public CRYPT_QUERY_INFO(string newAlgoName, int newBlockSize, int newMinKeySize, int newKeySize,
            int newMaxKeySize)
        {
    ...
    public class CryptException : ApplicationException
    {
        ...
        public int Status
        {
            get { return (int) Data["Status"]; }
        }

        public int ExtraInfo
        {
            get { return (int) Data["ExtraInfo"]; }
        }

        private static string convertMessage(int status)
        {
            var prefix = Convert.ToString(status) + ": ";
            switch (status)
            {
                case Cryptography.ERROR_PARAM1:
                    return prefix + "Bad argument, parameter 1";
                case Cryptography.ERROR_PARAM2:
                    return prefix + "Bad argument, parameter 2";
                case Cryptography.ERROR_PARAM3:
                    return prefix + "Bad argument, parameter 3";
                case Cryptography.ERROR_PARAM4:
                    return prefix + "Bad argument, parameter 4";
                case Cryptography.ERROR_PARAM5:
                    return prefix + "Bad argument, parameter 5";
                case Cryptography.ERROR_PARAM6:
                    return prefix + "Bad argument, parameter 6";
                case Cryptography.ERROR_PARAM7:
                    return prefix + "Bad argument, parameter 7";
                case Cryptography.ERROR_MEMORY:
                    return prefix + "Out of memory";
                case Cryptography.ERROR_NOTINITED:
                    return prefix + "Data has not been initialised";
                case Cryptography.ERROR_INITED:
                    return prefix + "Data has already been init'd";
                case Cryptography.ERROR_NOSECURE:
                    return prefix + "Opn.not avail.at requested sec.level";
                case Cryptography.ERROR_RANDOM:
                    return prefix + "No reliable random data available";
                case Cryptography.ERROR_FAILED:
                    return prefix + "Operation failed";
                case Cryptography.ERROR_INTERNAL:
                    return prefix + "Internal consistency check failed";
                case Cryptography.ERROR_NOTAVAIL:
                    return prefix + "This type of opn.not available";
                case Cryptography.ERROR_PERMISSION:
                    return prefix + "No permiss.to perform this operation";
                case Cryptography.ERROR_WRONGKEY:
                    return prefix + "Incorrect key used to decrypt data";
                case Cryptography.ERROR_INCOMPLETE:
                    return prefix + "Operation incomplete/still in progress";
                case Cryptography.ERROR_COMPLETE:
                    return prefix + "Operation complete/can't continue";
                case Cryptography.ERROR_TIMEOUT:
                    return prefix + "Operation timed out before completion";
                case Cryptography.ERROR_INVALID:
                    return prefix + "Invalid/inconsistent information";
                case Cryptography.ERROR_SIGNALLED:
                    return prefix + "Resource destroyed by extnl.event";
                case Cryptography.ERROR_OVERFLOW:
                    return prefix + "Resources/space exhausted";
                case Cryptography.ERROR_UNDERFLOW:
                    return prefix + "Not enough data available";
                case Cryptography.ERROR_BADDATA:
                    return prefix + "Bad/unrecognised data format";
                case Cryptography.ERROR_SIGNATURE:
                    return prefix + "Signature/integrity check failed";
                case Cryptography.ERROR_OPEN:
                    return prefix + "Cannot open object";
                case Cryptography.ERROR_READ:
                    return prefix + "Cannot read item from object";
                case Cryptography.ERROR_WRITE:
                    return prefix + "Cannot write item to object";
                case Cryptography.ERROR_NOTFOUND:
                    return prefix + "Requested item not found in object";
                case Cryptography.ERROR_DUPLICATE:
                    return prefix + "Item already present in object";
                case Cryptography.ENVELOPE_RESOURCE:
                    return prefix + "Need resource to proceed";
                default:
