...
/* Checksum a static bignum, which is one where the bignum image is stored
   ...
   The principal alternative is CityHash (and its follow-on FarmHash),
   but that's incredibly complex (FarmHash is 12K LOC), and in any case 
   CityHash only exists as 64- and 128-bit variants, there's no 32-bit 
   version of the hash.

   The following code is based on the MurmurHash3 implementation by Austin 
   Appleby, who placed it in the public domain */

#ifdef _MSC_VER
  #define ROTL32( x, r )	_rotl( x, r )
  #define ROTL64( x, r )	_rotl64( x, r )
#else
  /* Most compilers should have rotate-recognisers, this is the form that 
     will produce a native rotate instruction */
