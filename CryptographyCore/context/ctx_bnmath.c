...
/* Zero-pad bignum data so that the smaller bignum is of the same size as 
   ...
   given the they've been manipulated by the OpenSSL code we make this 
...
BOOLEAN BN_lshift( INOUT BIGNUM *r, const BIGNUM *a, 
	...
	REQUIRES_B( wordShiftAmount + a->top < getBNMaxSize( r ) );
	...
	if( bitShiftAmount == 0 )
		{
		/* Move the bits up, starting from the MSB and moving down to the 
		   LSB */
		for( i = a->top - 1; i >= 0; i-- )
			rData[ wordShiftAmount + i ] = aData[ i ];

		/* Clear the space that we've shifted up from.  This can become an 
		   issue when r == a */
...
BOOLEAN BN_rshift( INOUT BIGNUM *r, const BIGNUM *a, 
	...
	if( bitShiftAmount == 0 ) 
		{
		/* Move the bits down, starting from the LSB and moving down to the 
		   MSB */
		...
		/* Clear the space that we've shifted down from.  This can become an 
		   issue when r == a */
		for( i = wordsToShift; i < wordsToShift + wordShiftAmount; i++ ) 
			rData[ i ] = 0;
