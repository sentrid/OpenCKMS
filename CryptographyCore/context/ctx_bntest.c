...
static BOOLEAN selfTestOp( const SELFTEST_VALUE *selftestValue,
	...
	switch( op )
		...
		case BN_OP_ADD:
			bnStatus = BN_add( &result, &a, &b );
			break;

		case BN_OP_SUB:
			BN_swap( &expectedResult, &a );
			bnStatus = BN_sub( &result, &a, &b );
			break;

		case BN_OP_LSHIFT:
			bnStatus = BN_lshift( &result, &a, bValue );
			break;

		case BN_OP_RSHIFT:
			BN_swap( &expectedResult, &a );
			bnStatus = BN_rshift( &result, &a, bValue );
			break;

		case BN_OP_ADDWORD:
			if( BN_copy( &result, &a ) == NULL )
				bnStatus = !BN_STATUS;
			if( bnStatusOK( bnStatus ) )
				bnStatus = BN_add_word( &result, bWord );
			break;

		case BN_OP_SUBWORD:
			if( BN_copy( &result, &expectedResult ) == NULL )
				bnStatus = !BN_STATUS;
			if( bnStatusOK( bnStatus ) )
				{
				if( BN_copy( &expectedResult, &a ) == NULL )
					bnStatus = !BN_STATUS;
				}
			if( bnStatusOK( bnStatus ) )
				bnStatus = BN_sub_word( &result, bWord );
			break;
		...
		case BN_OP_MULWORD:
			if( BN_copy( &result, &a ) == NULL )
				bnStatus = !BN_STATUS;
			if( bnStatusOK( bnStatus ) )
				bnStatus = BN_mul_word( &result, bWord );
			break;

#if defined( USE_ECDH ) || defined( USE_ECDSA )
		case BN_OP_MODADD:
			bnStatus = BN_mod_add_quick( &result, &a, &b, &mod );
			break;

		case BN_OP_MODSUB:
			bnStatus = BN_mod_sub_quick( &result, &a, &b, &mod );
			break;

		case BN_OP_MODMUL:
			bnStatus = BN_mod_mul( &result, &a, &b, &mod, &bnCTX );
			break;
		
		case BN_OP_MODSHIFT:
			bnStatus = BN_mod_lshift_quick( &result, &a, bValue, &mod );
			break;
#endif /* USE_ECDH || USE_ECDSA */
