...
#define ASSERT_CONCAT( a, b )	ASSERT_CONCAT_( a, b )
  ...
  /* Supposedly built into gcc 4.5 and above (as usual for new gcc features
	 ...
	 gcc 4.9 and has an effect there, and seems to work in 4.7 and 4.8 as 
	 well.  To protect ourselves, we assume it's present in 4.7 and above 
	 but also check for static_assert being defined, which indicates its 
	 presence in an earlier version */
  #define static_assert_opt( expr, string ) \
		  assert( expr )
