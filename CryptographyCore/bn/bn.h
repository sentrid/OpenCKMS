/* cryptlib uses some of the mid-level OpenSSL bignum routines, but nothing 
   ...
   bn_orig.h to ensure that the original OpenSSL copyright isn't impacted,
...
/* 64-bit architectures where long long is 64 bits but long is 32 bits,
   ...
   That there is pseudo-support for an __int128 type in VC++ and gcc but 
   it's still too preliminary to rely on */
...
typedef struct {
	/* Bignums */
    BIGNUM RR;		/* Used as a temp.in BN_mont_ctx_set() */
    BIGNUM N;		/* Modulus */
    BN_ULONG n0[ 2 ];/* Low words of some intermediate value from
						BN_MONT_CTX_set() */
    int ri;			/* No.bits in N */
    int flags;
	} BN_MONT_CTX;
